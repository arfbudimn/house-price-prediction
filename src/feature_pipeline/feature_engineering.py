...
def run_feature_engineering(
    ...
    df_train = pd.read_csv(in_train_path)
    df_eval = pd.read_csv(in_eval_path)
    df_holdout = pd.read_csv(in_holdout_path)

    print("Train date range:", df_train["date"].min(), "to", df_train["date"].max())
    print("Eval date range:", df_eval["date"].min(), "to", df_eval["date"].max())
    print("Holdout date range:", df_holdout["date"].min(), "to", df_holdout["date"].max())

    # Date features
    df_train = add_date_features(df_train)
    df_eval = add_date_features(df_eval)
    df_holdout = add_date_features(df_holdout)

    # Frequency encode zipcode (fit on train only)
    freq_map = None
    if "zipcode" in df_train.columns:
        df_train, df_eval, freq_map = frequency_encode(df_train, df_eval, "zipcode")
        df_holdout["zipcode_freq"] = df_holdout["zipcode"].map(freq_map).fillna(0)
        dump(freq_map, MODELS_DIR / "freq_encoder.pkl")   # save mapping

    # Target encode city_full (fit on train only)
    target_encoder = None
    if "city_full" in df_train.columns:
        df_train, df_eval, target_encoder = target_encode(df_train, df_eval, "city_full", "price")
        df_holdout["city_full_encoded"] = target_encoder.transform(df_holdout["city_full"])
        dump(target_encoder, MODELS_DIR / "target_encoder.pkl")  # save encoder

    # Drop leakage / raw categoricals
    train_df, eval_df = drop_unused_columns(df_train, df_eval)
    holdout_df, _ = drop_unused_columns(df_holdout.copy(), df_holdout.copy())
